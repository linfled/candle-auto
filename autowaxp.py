...
while True:
    ...
    df = pyupbit.get_ohlcv("KRW-WAXP", interval="minute240")  # 4시간봉 데이터 가져오기
    ...
    last_ma10 = ma10[-2]  # 전 봉 28일 평균선

    avg = last_ma1 - last_ma5   # 현재봉 - 전봉 14일 평균선

    current_price = pyupbit.get_current_price("KRW-WAXP")
    print("**********************")
    ...
    try:
        if last_ma5 < last_ma10 and avg > 0:
            ...
            print("현재 원화 잔고는", krw, "입니다")
            if krw > 0:
                upbit.buy_market_order("KRW-WAXP", krw)
                print("구매 완료")
                print(current_price, "구매가격")
                print("======================================")

        if last_ma5 > last_ma10 and avg < 0:
            btc = get_balance("WAXP")
            print("현재 가상화폐 잔고는", btc, "입니다")
            upbit.sell_market_order("KRW-WAXP", btc)
            print("판매 완료")
