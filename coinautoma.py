...
df = pyupbit.get_ohlcv("KRW-BTC", interval="minute240")    # 1시간봉 데이터 가져오기
...
while True:
    ...
    try:
        if last_ma5 > last_ma10:   # 매수   7일선이 14일선보다 위에 있을때
            ...
            print("현재 원화 잔고는", krw, "입니다")
            if krw > 5000:
                upbit.buy_market_order("KRW-BTC", krw * 0.7)
                print("구매 완료")
                print("======================================")
                t5 = datetime.now()
                delta2 = t5 - t1
                time.sleep(3600 - delta2.total_seconds())
                continue
            else:
        ...
        if last_ma5 < last_ma10:  # 매도    7일선이 14일선보다 아래에 있을때
            ...
            t4 = datetime.now()
            delta3 = t4 - t1
            time.sleep(3600 - delta3.total_seconds())
            continue
        else:
            continue
